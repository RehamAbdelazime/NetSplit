...
)
{
    ...
    for (ULONG i = 0; i < ifTable->NumEntries; i++)
    {
        ...
        for (ULONG j = 0; j < addrTable->NumEntries; j++)
        {
            ...
            DbgPrintEx(
                ...
                DPFLTR_INFO_LEVEL,
                "NetSplitProof: adapter '%ws' idx=%lu type=%lu up, IPv4=%u.%u.%u.%u\n",
                ifRow->Alias,
    ...
    if (!foundWiFi)
    {
        DbgPrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "NetSplitProof: no active Wi-Fi adapter with an IPv4 address found\n");
    }

    if (!foundEthernet)
    {
        DbgPrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "NetSplitProof: no active Ethernet adapter with an IPv4 address found\n");
    }
